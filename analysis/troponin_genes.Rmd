...
foreach(pname=names(de), .combine = bind_rows) %do% {
  ...
  ggplot(aes(rep, factor(discovery,rev(concs)), fill=reprate, label=format(reprate,digits=2))) + geom_tile() + geom_text() + scale_fill_gradient(limits=c(0,1), low="blue", high="white", guide="none") + ylab("Discovery concentration (q<0.05)") + xlab("Replication concentration (p<0.05)")
```
...
```{r}
z= for_sigma  %>% select(gene,conc,se) %>% spread(conc, se) %>% mutate(gene=NULL) %>% as.matrix()
```
