...
eagle2=stan_model("~/Dropbox/eagle/eagle/beta_binomial_models/eagle2_norm.stan")
...
results=setNames( foreach(gene=genes, .errorhandling=errorhandling) %do% {
  ...
  dat$ys=array(0,c(46,5,0))
  dat$ns=array(0,c(46,5,0))
  ...
  res=rbindlist( foreach(cis_snp=cis_snps, .errorhandling=errorhandling) %dopar% {
   ...
   # ggplot(lin_data, aes(treat, col=as.factor(geno))) + geom_boxplot(aes(y=fitted), outlier.shape = NA) + geom_point(aes(y=count), position = position_dodge(width=2/3),alpha=.5)
    ...
    #w=with(lin_data, 1/(count / lib_size^2))
    #anova(lm(count ~ treat + geno, data=lin_data, weights = w),lm(count ~ treat * geno, data=lin_data, weights = w))

    #ggplot(lin_data, aes(treat, ynorm, col=as.factor(geno))) + geom_boxplot(outlier.shape = NA) + geom_point(position = position_dodge(width=4/5),alpha=.5) + theme_bw()
