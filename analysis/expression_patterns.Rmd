...
all_go_sub = all_go_sub %>% group_by(cluster) %>% mutate(p_adjust=p.adjust(Enrichment, method="BH")) %>% ungroup()
```

```{r}
gz=gzfile("../supp_data/pathway_enrich.txt.gz","w")
all_go_sub %>% filter(p_adjust<0.05) %>% arrange(p_adjust) %>% mutate(cluster=factor(cluster, 1:6, clusters)) %>% select(cluster, TermID, Term, odds_ratio=OR, p=Enrichment, p_adjust) %>% as.data.frame() %>% format(digits=5) %>% write.table(gz, quote=F,row.names=F, col.names=T, sep="\t")
close(gz)
```
...
```{r}
gz=gzfile("../supp_data/motif_enrich.txt.gz","w")
all_tf %>% filter(`q-value (Benjamini)` < 0.5) %>% arrange(`q-value (Benjamini)`) %>% mutate(cluster=factor(cluster, 1:6, clusters)) %>% select(cluster, motif_info=`Motif Name`, motif, `Consensus`, odds_ratio=OR, log_p=`Log P-value`, p_adjust=`q-value (Benjamini)`) %>% as.data.frame() %>% format(digits=5) %>% write.table(gz, quote=F,row.names=F, col.names=T, sep="\t")
close(gz)
```