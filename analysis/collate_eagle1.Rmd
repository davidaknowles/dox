...
```{r}
res_dir="/scratch/users/dak33/data/dox_data/eagle1/"
...
re_ase_qtl = ase_qtl %>% rename(p=p_interact, cis_snp=snp) %>% filter(!is.na(p)) %>% bonferroni() %>% ungroup()
...
ase_qtl = ase_qtl %>% inner_join( snploc, by=c("chr", snp="pos"))
