...
o=optimizing(sm, data, verbose=T, iter=100,  as_vector=F) # takes a while... 

ggplot( data.frame(n=1:20, p=sort(o$par$p, decreasing = T)), aes(n,p)) + geom_bar(stat="identity") + theme_bw(base_size = 16) + ylab("Mixture weight") + xlab("Mixture component")
```

Take top six clusters
```{r}
to_use=order(o$par$p, decreasing = T)[1:6]
mix_means=o$par$betas[to_use,]

sm=stan_model("mixlm_pred.stan")
o_pred=optimizing(sm, data, init=o$par, verbose=T, iter=1,  as_vector=F)
lp=o_pred$par$logprob
assignments=apply(lp, 1, which.max)
# save(file="../data/expression_patterns.RData", o, assignments)
