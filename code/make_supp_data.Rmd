...
```{r}
anno_input = anno %>% select(sample=sample, cell_line=individual, individual=findiv,  conc=conc)
...
require(stringr)
lf_anno=str_split_fixed(colnames(lf_counts),"_",2) %>% as.data.frame(stringsAsFactors=F) %>% set_colnames(c("findiv","conc")) %>% left_join(df, by="findiv")
