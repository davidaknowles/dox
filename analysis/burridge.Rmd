...
```{r}
require(broom)
data.frame( findiv=as.integer(rownames(pred_doxtox)), doxtox=pred_doxtox[,1]) %>% left_join(anno %>% filter(conc==5.), by="findiv") %>% do(tidy(cor.test(.$doxtox,.$troponin,method="spearman")))
```
...
doxtox_pred_trop = data.frame( findiv=rownames(pred_doxtox), doxtox=pred_doxtox[,1], stringsAsFactors = F) %>% left_join(troponin_rate %>% mutate(individual=as.character(individual)), by=c("findiv"="individual")) 
